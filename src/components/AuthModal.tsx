...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs animate-fade-in">
      <div className="bg-white border border-[#e6e6e6] rounded-2xl w-full max-w-md overflow-hidden shadow-notion-modal flex flex-col max-h-[90vh] text-[#31302e]">
        {/* Header */}
        <div className="relative px-6 pt-5 pb-4 border-b border-[#e6e6e6] bg-white">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-[#0075de]/10 border border-[#0075de]/20 flex items-center justify-center text-[#0075de]">
                <Sparkles className="w-4 h-4" />
              </div>
              <div>
                <h2 className="text-base font-bold text-[#000000] tracking-tight">เข้าสู่ระบบ / สมัครสมาชิก</h2>
                <p className="text-xs text-[#615d59]">สัมผัสประสบการณ์ฟังเพลงร่วมกันบน pleng.online</p>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="p-6 space-y-5 overflow-y-auto">
          ...
          <div className="p-3.5 rounded-xl bg-[#f6f5f4] border border-[#e6e6e6] flex items-center gap-3 shadow-xs">
            <div
              className="w-11 h-11 rounded-full overflow-hidden border-2 shrink-0 relative bg-white shadow-xs"
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-[#000000] truncate">{currentUser.name}</span>
                {currentUser.provider === 'google' && (
                  <span className="px-2 py-0.2 rounded-full bg-rose-50 text-rose-600 border border-rose-200 text-[10px] font-medium">
                    Google
                  </span>
                )}
                {currentUser.provider === 'facebook' && (
                  <span className="px-2 py-0.2 rounded-full bg-blue-50 text-[#1877F2] border border-blue-200 text-[10px] font-medium">
                    Facebook
                  </span>
                )}
                {(!currentUser.provider || currentUser.provider === 'guest') && (
                  <span className="px-2 py-0.2 rounded-full bg-white text-[#615d59] border border-[#e6e6e6] text-[10px] font-medium">
                    Guest Mode
                  </span>
                )}
              </div>
              <p className="text-xs text-[#615d59] truncate mt-0.5">
          ...
          <div className="p-4 rounded-xl bg-[#f6f5f4] border border-[#e6e6e6] space-y-2.5">
            ...
            <div className="grid grid-cols-1 gap-2 text-xs text-[#615d59]">
              ...
              <div className="flex items-center gap-2">
                <Music2 className="w-3.5 h-3.5 text-[#391c57] shrink-0" />
